...
bool CSpreadSheet::AddHeaders(CStringArray &FieldNames, bool replace)
{
	if (m_bAppend) // Append to old Sheet
	{
		...
		if (ReadRow(m_atempArray, 1)) // Add new columns
		{
			...
			{
				m_dTotalColumns = (short)m_atempArray.GetSize();
			}
			return true;
		}
		return false;				
	...
	{
		m_dTotalColumns = (short)FieldNames.GetSize();
		if (!AddRow(FieldNames, 1, true))
		{
			return false;
		}
...
bool CSpreadSheet::AddRow(CStringArray &RowValues, long row, bool replace)
{
	...
	if (row == 1)
	{
		if (m_bExcel) 
		{
			...
			m_dTotalColumns = (short)RowValues.GetSize();
		}
...
bool CSpreadSheet::Commit()
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		...
		try
		{
			...
			if (!m_bAppend)
			{
				m_dTotalColumns = (short)m_aFieldNames.GetSize();
				m_bAppend = true;
			}
		}
		catch(CDBException *e)
		...
		try
		{
			...
			if (File != NULL)
			{
				...
				if (Archive != NULL)
				{
					...
					m_dTotalRows = (long)m_aRows.GetSize();
