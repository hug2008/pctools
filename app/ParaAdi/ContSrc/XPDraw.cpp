...
/////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////
#define DB_XPDRAW_GETTYPE(Style)       ((Style)&0x00FF)
#define DB_XPDRAW_ISENABLED(Style)     (!((Style)&DB_XPDRAW_DISABLED))
#define DB_XPDRAW_ISBORDER(Style)      (((Style)&DB_XPDRAW_BORDER)==DB_XPDRAW_BORDER)
#define DB_XPDRAW_ISWINDOWDC(Style)    (((Style)&DB_XPDRAW_WINDOWDC)==DB_XPDRAW_WINDOWDC)
#define DB_XPDRAW_ISFLAT(Style)        (((Style)&DB_XPDRAW_FLAT)==DB_XPDRAW_FLAT)
#define DB_XPDRAW_ISPRESSED(Style)     (((Style)&(DB_XPDRAW_PRESSED|DB_XPDRAW_PRESSED2))!=0)
#define DB_XPDRAW_ISPRESSED1(Style)    (((Style)&DB_XPDRAW_PRESSED)==DB_XPDRAW_PRESSED)
#define DB_XPDRAW_ISPRESSED2(Style)    (((Style)&DB_XPDRAW_PRESSED2)==DB_XPDRAW_PRESSED2)
#define DB_XPDRAW_ISOVER(Style)        (((Style)&DB_XPDRAW_OVER)==DB_XPDRAW_OVER)
#define DB_XPDRAW_ISTRANSPARENT(Style) (((Style)&DB_XPDRAW_TRANSPARENT)==DB_XPDRAW_TRANSPARENT)
...
void CDrawButton::Draw (CDC* pDC, DWORD wStyle) const
{
    ...
    if ( DB_XPDRAW_ISBORDER(m_wStyle) && !DB_XPDRAW_ISFLAT(wStyle) )
    {
    ...
    if ( DB_XPDRAW_ISFLAT(wStyle) )
    {
        crBorder = ::GetSysColor (COLOR_HIGHLIGHT);
        crPressed = HLS_TRANSFORM (crBorder, +50, -50);
        crOver = HLS_TRANSFORM (crBorder, +70, -57);
    }
    if ( DB_XPDRAW_GETTYPE(wStyle) == DB_XPDRAW_UPDOWN )
    {
        if ( !DB_XPDRAW_ISPRESSED2(wStyle) )
        {
            if ( DB_XPDRAW_ISFLAT(wStyle) )
            {
                CRect rcBtn (rc.left, rc.top, rc.right, (rc.top+rc.bottom)/2+1);

                if ( DB_XPDRAW_ISENABLED(wStyle) && DB_XPDRAW_ISOVER(wStyle) )
                {
                    pen.Color (crBorder);
                    brush.Color (DB_XPDRAW_ISPRESSED(wStyle) ? crPressed : crOver);
                    pDC->Rectangle (rcBtn);
                }
                else if ( !DB_XPDRAW_ISTRANSPARENT(wStyle) )
                {
                    pen.Color (::GetSysColor (DB_XPDRAW_ISBORDER(m_wStyle) ? COLOR_3DSHADOW : COLOR_WINDOW));
                    pDC->Rectangle (rcBtn);
                }
            }
            else
            {
                ...
                pDC->DrawFrameControl (rcBtn, DFC_BUTTON, DFCS_BUTTONPUSH|(DB_XPDRAW_ISPRESSED1(wStyle) ? DFCS_PUSHED : 0));
            }
        }
        if ( !DB_XPDRAW_ISPRESSED(wStyle) && !DB_XPDRAW_ISFLAT(wStyle) )
        {
            pDC->MoveTo (rc.left, (rc.top+rc.bottom)/2);
            pDC->LineTo (rc.right, (rc.top+rc.bottom)/2);
        }
        if ( !DB_XPDRAW_ISPRESSED1(wStyle) )
        {
            if ( DB_XPDRAW_ISFLAT(wStyle) )
            {
            ...
            else
            {
                ...
                pDC->DrawFrameControl (rcBtn, DFC_BUTTON, DFCS_BUTTONPUSH|(DB_XPDRAW_ISPRESSED2(wStyle) ? DFCS_PUSHED : 0));
            }
        }
        if ( DB_XPDRAW_ISFLAT(wStyle) )
        {
            crText = DB_XPDRAW_ISENABLED(wStyle) && pDC->GetPixel (rc.left+1, rc.top+1) == crOver ? RGB(0,0,0) : ::GetSysColor (COLOR_BTNTEXT);
            pen.Color (DB_XPDRAW_ISPRESSED(wStyle) ? RGB(240,240,240) : DB_XPDRAW_ISENABLED(wStyle) ? crText : ::GetSysColor (COLOR_GRAYTEXT));
        }
        else
        {
            pen.Color (::GetSysColor (DB_XPDRAW_ISENABLED(wStyle) ? COLOR_BTNTEXT : COLOR_GRAYTEXT));
        }
        if ( !DB_XPDRAW_ISPRESSED2(wStyle) )
        {
            _DrawTriangle (pDC, (rc.left+rc.right)/2 + ((DB_XPDRAW_ISPRESSED1(wStyle)&&!DB_XPDRAW_ISFLAT(wStyle)) ? 1 : 0),
                                (rc.top*3+rc.bottom)/4 + ((DB_XPDRAW_ISPRESSED1(wStyle)||DB_XPDRAW_ISFLAT(wStyle)) ? 1 : 0) - 2, 3, false);
        }
        if ( !DB_XPDRAW_ISPRESSED1(wStyle) )
        {
            _DrawTriangle (pDC, (rc.left+rc.right)/2 + ((DB_XPDRAW_ISPRESSED2(wStyle)&&!DB_XPDRAW_ISFLAT(wStyle)) ? 1 : 0),
                                (rc.top+rc.bottom*3)/4 + ((DB_XPDRAW_ISPRESSED2(wStyle)&&!DB_XPDRAW_ISFLAT(wStyle)) ? 1 : 0) + 1, 3, true);
        }
    }
    else
    {
        ...
        {
            pDC->DrawFrameControl (rc, DFC_BUTTON, DFCS_BUTTONPUSH|(DB_XPDRAW_ISPRESSED(wStyle) ? DFCS_PUSHED : 0));
        }
        switch ( DB_XPDRAW_GETTYPE(wStyle) )
        {
            case DB_XPDRAW_3POINTS:
                if ( DB_XPDRAW_ISFLAT(wStyle) )
                {
                    crText = DB_XPDRAW_ISENABLED(wStyle) && pDC->GetPixel (rc.left+1, rc.top+1) == crOver ? RGB(0,0,0) : ::GetSysColor (COLOR_BTNTEXT);
                    crText = DB_XPDRAW_ISPRESSED(wStyle) ? RGB(240,240,240) : DB_XPDRAW_ISENABLED(wStyle) ? crText : ::GetSysColor (COLOR_GRAYTEXT);
                }
                else
                {
                    crText = ::GetSysColor (DB_XPDRAW_ISENABLED(wStyle) ? COLOR_BTNTEXT : COLOR_GRAYTEXT);
                }
                crText = pDC->SetTextColor (crText);
                pDC->SetBkMode (TRANSPARENT);
                pDC->DrawText (_T("..."), 3, CRect(rc.left + ((DB_XPDRAW_ISPRESSED(wStyle)||DB_XPDRAW_ISFLAT(wStyle)) ? 2 : 1),
                                                   rc.top, rc.right, rc.bottom + ((DB_XPDRAW_ISPRESSED(wStyle)&&!DB_XPDRAW_ISFLAT(wStyle)) ? 1 : 0)),
            ...
            case DB_XPDRAW_DOWN:
                ...
                _DrawTriangle (pDC, (rc.left+rc.right)/2 + ((DB_XPDRAW_ISPRESSED(wStyle)&&!DB_XPDRAW_ISFLAT(wStyle)) ? 1 : 0),
                                    (rc.top+rc.bottom)/2 + ((DB_XPDRAW_ISPRESSED(wStyle)||DB_XPDRAW_ISFLAT(wStyle)) ? 2 : 1) - ((DB_XPDRAW_GETTYPE(wStyle) == DB_XPDRAW_UP) ? 3 : 0),
                                    4, DB_XPDRAW_GETTYPE(wStyle) == DB_XPDRAW_DOWN);
                break;

            case DB_XPDRAW_CROSS:
                {
                    ...
                    else if ( DB_XPDRAW_ISPRESSED(wStyle) )
                    {
                        x1++; y1++;
                        x2++; y2++;
                    }
                    if ( DB_XPDRAW_ISFLAT(wStyle) )
                    {
                        crText = DB_XPDRAW_ISENABLED(wStyle) && pDC->GetPixel (x1, y1+1) == crOver ? RGB(0,0,0) : ::GetSysColor (COLOR_BTNTEXT);
                        pen.Color (DB_XPDRAW_ISPRESSED(wStyle) ? RGB(240,240,240) : DB_XPDRAW_ISENABLED(wStyle) ? crText : ::GetSysColor (COLOR_GRAYTEXT));
                    }
                    else
                    {
...
DWORD CDrawButton::Click (CWnd* pWnd, CPoint pt, UINT nIDRepeat) const
{
    ...
    if ( ::GetCapture() != NULL || !DB_XPDRAW_ISENABLED(m_wStyle) )
    {
        return DB_XPDRAW_DEFAULT;
    }
    ...
    int   nSpinType = DB_XPDRAW_PRESSED;
    CRect rcBtn(m_Rect);

    if ( DB_XPDRAW_GETTYPE(m_wStyle) == DB_XPDRAW_UPDOWN )
    {
        if ( pt.y >= (rcBtn.top+rcBtn.bottom)/2 )
        {
            nSpinType = DB_XPDRAW_PRESSED2;
        ...
        if ( nIDRepeat != 0 )
        {
            s_wNotify = (nSpinType == DB_XPDRAW_PRESSED) ? LOWORD(nIDRepeat) : HIWORD(nIDRepeat);
    ...
    while ( true )
    {
        ...
        if ( bNewDown != bDown )
        {
            ...
            if ( DB_XPDRAW_ISWINDOWDC(m_wStyle) )
            {
                CWindowDC cDC (pWnd);
                Draw (&cDC, m_wStyle|(bDown ? nSpinType : DB_XPDRAW_DEFAULT));
            }
            else
            {
                CClientDC cDC (pWnd);
...
ExitLoop:
    ReleaseCapture();
    if ( DB_XPDRAW_GETTYPE(m_wStyle) == DB_XPDRAW_UPDOWN && nIDRepeat != 0 )
    {
    ...
    return bClick ? (DB_XPDRAW_GETTYPE(m_wStyle)|nSpinType) : DB_XPDRAW_DEFAULT;
}
...
{
    if ( DB_XPDRAW_ISFLAT(m_wStyle) )
    {
        if ( !DB_XPDRAW_ISOVER(m_wStyle) && PtInRect (pt) )
        {
            if ( pWnd->SetTimer (54321, 300, &CDrawButton_TimerProcMouseOver) )
            {
                m_wStyle |= DB_XPDRAW_OVER;
        ...
        else if ( DB_XPDRAW_ISOVER(m_wStyle) && !PtInRect (pt) )
        {
