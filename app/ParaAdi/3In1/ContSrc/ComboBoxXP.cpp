...
CComboBoxXP::CComboBoxXP () :
    m_nVisibleLines (10), m_nDefaultHeight (-1), m_nEditHeight (-1)
{
}
...
void CComboBoxXP::PreSubclassWindow ()
{
    ...
    SetWindowPos (NULL, 0, 0, rc.Width(), 1000, SWP_NOMOVE|SWP_NOZORDER);
}
...
void CComboBoxXP::OnPaint ()
{
    ...
    CClientDC cDC (this);
    DWORD wStyle = DB_XPDRAW_FLAT|DB_XPDRAW_DOWN|(IsWindowEnabled() ? DB_XPDRAW_ENABLED : DB_XPDRAW_DISABLED);
    ...
    if ( !bInToolbar )
    {
        wStyle |= DB_XPDRAW_BORDER;
    }
    if ( bFocus || m_MouseMgr.MouseOver() )
    {
        wStyle |= DB_XPDRAW_OVER;
    }
    if ( bDropped )
    {
        wStyle |= DB_XPDRAW_OVER|DB_XPDRAW_PRESSED;
    }
