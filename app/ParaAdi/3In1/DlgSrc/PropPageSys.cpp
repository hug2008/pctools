...
void CPropPageSys::OnSaveData(void)
{
	...
	UpdateData(TRUE);
	// 1 -- the combo box
	m_pParentWnd->szDialWaitingTime02.Format("0%d", m_nAutoDelay);

	// 2 -- the edit
	m_pParentWnd->szMscNum04 = m_strSMSCenter;

	// 3 -- the check button
	m_pParentWnd->szAddZeroIfOtherCity4000 = 
		((CButton*)GetDlgItem(IDC_PAGE_1_RADIO_ADDYES))->GetCheck() ? "01" : "00";

	// 4 -- the check button
	m_pParentWnd->szBaringOtherCityNum05 = 
...
void CPropPageSys::OnRefreshPage(void)
{
	...
	// 1 -- the combo box
	m_nAutoDelay = atoi(m_pParentWnd->szDialWaitingTime02);

	// 2 -- the edit
	m_strSMSCenter = m_pParentWnd->szMscNum04;

	// 3 -- the check button
	if (atoi(m_pParentWnd->szAddZeroIfOtherCity4000))
	{
	...
	// 4 -- the check button
	if (atoi(m_pParentWnd->szBaringOtherCityNum05))
	{
