...
BOOL CPropPageHeader::OnInitDialog() 
{
	...
	szTips.Format(TIP_MOBILE_LTEXT, MOBILE_HEAD_TOTAL);
...
BOOL CPropPageHeader::OnCloseTest(void)
{
	...
	for (Index=0; Index<MOBILE_HEAD_TOTAL/2; Index++)
	{
		szItem1 = m_rcMob.GetItemText(Index, 1);
		szItem3 = m_rcMob.GetItemText(Index, 3);

		if (szItem1.GetLength() > MOBILE_HEAD_LEN)
		{
			szErrMsg.Format(_T("The header no more than %d numbers, (No:%d)"), MOBILE_HEAD_LEN, Index*2 + 1);
		...
		if (szItem3.GetLength() > MOBILE_HEAD_LEN)
		{
			szErrMsg.Format(_T("The header no more than %d numbers, (No:%d)"), MOBILE_HEAD_LEN, Index*2 + 2);
			m_pParentWnd->SendMessage(WM_MSG_ACTIVE_THIS_PAGE_NOTIFY, IDD_PROPPAGE_MOBILE_HEADER);
			m_rcMob.SetFocus();
			m_rcMob.SetSelection(Index);
			MessageBox(szErrMsg, SYS_NAME, MB_ICONSTOP);
...
void CPropPageHeader::OnSaveData(void)
{
	...
	for (Index=0; Index<MOBILE_HEAD_TOTAL; Index++)
	{
		...
		{
			m_pParentWnd->szMobileHeader14001[Index] = szAry.GetAt(Index);
		}
		else
		{
			m_pParentWnd->szMobileHeader14001[Index].Empty();
		}
	}
#endif
}
...
void CPropPageHeader::OnRefreshPage(void)
{
	...
	for (Index=0; Index<MOBILE_HEAD_TOTAL; Index++)
	{
		if (!m_pParentWnd->szMobileHeader14001[Index].IsEmpty())
		{
			szAry.Add(m_pParentWnd->szMobileHeader14001[Index]);
		}

		if (Index < MOBILE_HEAD_TOTAL/2)
		{
