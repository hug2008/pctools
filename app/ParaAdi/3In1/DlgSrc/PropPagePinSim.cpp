...
void CPropPagePinSim::DoDataExchange(CDataExchange* pDX)
{
	...
	//}}AFX_DATA_MAP
	DDX_Control(pDX, IDC_EDIT_USERTAG, m_eUserTag);
	DDX_Text(pDX, IDC_EDIT_USERTAG, m_szUserTag);
}
...
//		(((CButton*)GetDlgItem(IDC_PAGE_8_RADIO_USEPINNO))->GetCheck())
	...
	if (((CButton*)GetDlgItem(IDC_PAGE_8_RADIO_SIMYES))->GetCheck() == TRUE)
	{
		...
		if (m_strSimCode.GetLength() == 0)
		{
			...
			((CEdit*)GetDlgItem(IDC_PAGE_8_EDIT_SIM))->SetSel(0,-1);
			GetDlgItem(IDC_PAGE_8_EDIT_SIM)->SetFocus();
			MessageBox(_T("Serial number must be setted!"),SYS_NAME,MB_ICONSTOP);
			return FALSE;
		}
#endif
	}
	...
	// 6- user tag
	if (6 != m_szUserTag.GetLength())
	{
		m_pParentWnd->SendMessage(WM_MSG_ACTIVE_THIS_PAGE_NOTIFY, (IDD_PROPPAGE_8_PIN_SIM));
		m_eUserTag.SetFocus();
		m_eUserTag.SetSel(0, -1);
		MessageBox(_T("User tag must be 6 chars!"),SYS_NAME,MB_ICONSTOP);
		return FALSE;
	}

	if (!m_szUserTag.IsNumberAlpha())
	{
		m_pParentWnd->SendMessage(WM_MSG_ACTIVE_THIS_PAGE_NOTIFY, (IDD_PROPPAGE_8_PIN_SIM));
		m_eUserTag.SetFocus();
		m_eUserTag.SetSel(0, -1);
		MessageBox(_T("User tag must be Number or Alpha!"),SYS_NAME,MB_ICONSTOP);
		return FALSE;
	}
...
void CPropPagePinSim::OnSaveData(void)
{
	...
	m_pParentWnd->szIsHandsetAutoLockPinEnable6000 = 
		((CButton*)GetDlgItem(IDC_PAGE_8_RADIO_PINYES))->GetCheck() ? "01" : "00";

	m_pParentWnd->szUseEncryptPinFlag6005 = 
		((CButton*)GetDlgItem(IDC_PAGE_8_RADIO_USEPINYES))->GetCheck() ? "01" : "00";

	m_pParentWnd->szPinLen6001.Format("0%d", m_nPinLength+4);
	m_pParentWnd->szInitPin6002 = m_strInitPinCode;
	m_pParentWnd->szFixPin6004    = m_strFixedPinCode;

	if (((CButton*)GetDlgItem(IDC_PAGE_8_RADIO_SIMYES))->GetCheck())
	{
		m_pParentWnd->szIsHandsetCanBeBindedToASim7000 = "01";
		m_pParentWnd->szLockedImsi7001 = m_strSimCode;
	}
	else
	{
		m_pParentWnd->szIsHandsetCanBeBindedToASim7000 = "00";
		m_pParentWnd->szLockedImsi7001 = "";
	}

	m_pParentWnd->szUserTag30 = m_szUserTag;
}
...
void CPropPagePinSim::OnRefreshPage(void)
{
	...
	// 1 -- the check button
	m_bAutoLockPin = atoi(m_pParentWnd->szIsHandsetAutoLockPinEnable6000);
	((CButton*)GetDlgItem(IDC_PAGE_8_RADIO_PINYES))->SetCheck(m_bAutoLockPin);
	((CButton*)GetDlgItem(IDC_PAGE_8_RADIO_PINNO ))->SetCheck(!m_bAutoLockPin);


	// 1' -- the check button
	m_bUseRandomPin = atoi(m_pParentWnd->szUseEncryptPinFlag6005);
	((CButton*)GetDlgItem(IDC_PAGE_8_RADIO_USEPINYES))->SetCheck(m_bUseRandomPin);
	((CButton*)GetDlgItem(IDC_PAGE_8_RADIO_USEPINNO ))->SetCheck(!m_bUseRandomPin);


	// 2 -- the combo box
	m_nPinLength = atoi(m_pParentWnd->szPinLen6001)-4;
	m_cbPinLength.EnableWindow(m_bUseRandomPin);
	//ASSERT((m_nPinLength>=0)&&(m_nPinLength<=4));

	// 3 -- the edit
	m_strInitPinCode  = m_pParentWnd->szInitPin6002;

	// 3' -- the edit
	m_strFixedPinCode = m_pParentWnd->szFixPin6004;



	// 4 -- the check button
	bFlag = atoi(m_pParentWnd->szIsHandsetCanBeBindedToASim7000);
	...
	// 5 -- the edit
	m_strSimCode = bFlag ? m_pParentWnd->szLockedImsi7001: "" ;
...
*/
	...
	m_szUserTag = m_pParentWnd->szUserTag30;
...
void CPropPagePinSim::OnLockPage(BOOL bLock)
{
	...
	GetDlgItem(IDC_EDIT_USERTAG)->EnableWindow(bLock);	
}
