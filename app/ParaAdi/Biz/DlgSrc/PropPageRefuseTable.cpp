...
void CPropPageRefuseTable::OnSaveData(void)
{
	...
	// 1 -- the combo box
	m_pParentWnd->m_strREFUSE_5000_bRefuseType.Format("0%d", m_nRefuseType);
	...
	for (int i=0; i<INT_INIT_LISTBOX_ITEMNUM; i++)
	{
		...
		m_pParentWnd->m_AryREFUSE_5020_strRefuseTable[i] = strDirectNum;
	}
}
...
{
	// 1 -- the combo box
	m_nRefuseType = atoi(m_pParentWnd->m_strREFUSE_5000_bRefuseType);
	...
	for (i=0; i<INT_INIT_LISTBOX_ITEMNUM; i++)
	{
		...
		strRefuseNum = m_pParentWnd->m_AryREFUSE_5020_strRefuseTable[i];
