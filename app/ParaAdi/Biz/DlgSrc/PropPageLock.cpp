...
BOOL CPropPageLock::OnInitDialog() 
{
	...
	((CButton*)GetDlgItem(IDC_PAGE_9_RADIO_PRVYES))->EnableWindow(FALSE);
	((CButton*)GetDlgItem(IDC_PAGE_9_RADIO_PRVNO ))->EnableWindow(FALSE);
	GetDlgItem(IDC_PAGE_9_EDIT_PROVIDER)->EnableWindow(FALSE);
#endif
		...
		for (int i=0; i< nValue; i++)
		{
			...
			if (!strInput.IsEmpty())	// 允许不输入，或输入不足
			{
				if (!strInput.IsHexNumber())
				{
					...
					sError.Format(_T("No.%d:Region must be  hex'chars"),i+1);
					MessageBox(sError, SYS_NAME,MB_ICONSTOP);
...
void CPropPageLock::OnSaveData(void)
{
...
#if !defined (LOCK_SERVICE_PROVIDER)
	m_pParentWnd->m_strLOCK_7002_bLockProvider = m_bLockProvider ? _T("01") : _T("00");
#if defined(R16_ATC)
	m_pParentWnd->m_strLOCK_7003_nProvider     = m_bLockProvider ? m_strProvider : _T("00000");
#else
	m_pParentWnd->m_strLOCK_7003_nProvider     = m_bLockProvider ? m_strProvider : _T("00");
#endif
#endif
	// 2. 是否锁定小区
	m_bLockRegion = ((CButton*)GetDlgItem(IDC_PAGE_9_RADIO_REGIONYES))->GetCheck();
	m_pParentWnd->m_strLOCK_8000_bLockRegion = m_bLockRegion ? _T("01") : _T("00");
	...
	if (m_bLockRegion)
	{
		...
		m_pParentWnd->m_strLOCK_8001_nRegionNum.Format("%02d", nRegionNum);

		// 4. 是否自动锁小区
		m_bAutoLockRegion = ((CButton*)GetDlgItem(IDC_PAGE_9_RADIO_AUTOLOCKYES))->GetCheck();
		m_pParentWnd->m_strLOCK_8002_bAutoLockRegion = m_bAutoLockRegion ? _T("01") : _T("00");
		...
		for (nIndex=0; nIndex<nRegionNum; nIndex++)
		{
			...
			m_pParentWnd->m_strLOCK_8003_strLockRegion += pCell;
		}
		m_pParentWnd->m_strLOCK_8003_strLockRegion = m_pParentWnd->m_strLOCK_8003_strLockRegion.Left(nRegionNum*4);
		m_pParentWnd->m_strLOCK_8003_strLockRegion.MakeUpper();
	}
	else
	{
		m_bAutoLockRegion                            = FALSE;
		m_pParentWnd->m_strLOCK_8001_nRegionNum      = _T("00");
		m_pParentWnd->m_strLOCK_8002_bAutoLockRegion = _T("00");
		m_pParentWnd->m_strLOCK_8003_strLockRegion.Empty();
	}	
}
...
void CPropPageLock::OnRefreshPage(void)
{
...
#if !defined (LOCK_SERVICE_PROVIDER)
	// 1 -- 是否锁定运营商
	bLockFlag = atoi(m_pParentWnd->m_strLOCK_7002_bLockProvider);
	...
	m_strProvider = m_pParentWnd->m_strLOCK_7003_nProvider;
#else
	((CButton*)GetDlgItem(IDC_PAGE_9_RADIO_PRVYES))->EnableWindow(FALSE);
	((CButton*)GetDlgItem(IDC_PAGE_9_RADIO_PRVNO ))->EnableWindow(FALSE);
	GetDlgItem(IDC_PAGE_9_EDIT_PROVIDER)->EnableWindow(FALSE);
#endif
	// 2 -- 是否锁小区
	m_bLockRegion     = atoi(m_pParentWnd->m_strLOCK_8000_bLockRegion);
	m_bAutoLockRegion = atoi(m_pParentWnd->m_strLOCK_8002_bAutoLockRegion);
	...
	nRegionNum = atoi(m_pParentWnd->m_strLOCK_8001_nRegionNum);
	nRegionNum = nRegionNum>MAXREGIONNUM ? MAXREGIONNUM : nRegionNum;
	
	SetDlgItemInt(IDC_PAGE_9_EDIT_REGIONNUM, nRegionNum);
	OnChangeEditRegionNum();
	
	nRegionNum = min(m_pParentWnd->m_strLOCK_8003_strLockRegion.GetLength()/4, nRegionNum);
	for (nIndex=0; nIndex<nRegionNum; nIndex++)
		m_rcLockRegion.SetItemText(nIndex, 1,m_pParentWnd->m_strLOCK_8003_strLockRegion.Mid(nIndex*4, 4)); 
	CDialog::UpdateData(FALSE);
}
...
#if defined (LOCK_SERVICE_PROVIDER)
	GetDlgItem(IDC_PAGE_9_RADIO_PRVYES)->EnableWindow(FALSE);
	GetDlgItem(IDC_PAGE_9_RADIO_PRVNO )->EnableWindow(FALSE);
	GetDlgItem(IDC_PAGE_9_EDIT_PROVIDER)->EnableWindow(FALSE);
#endif
}
