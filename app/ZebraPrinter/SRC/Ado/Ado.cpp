...
BOOL CAdoConnection::ConnectAccess(CString dbpath, CString pass, long lOptions)
{
	...
	if (pass.GetLength() > 0) {
		strConnect.Format(ACCESS_CONN_USER, dbpath, pass);
	}else{
		strConnect.Format(ACCESS_CONN, dbpath);
	}
...
/*======================================================================== 
   name:  _variant_t数据格式的数据转换为字符串格式
=========================================================================*/
CString CAdoConnection::vartostr(_variant_t& var)
{
	CString strValue;
	switch(var.vt)
	{
...
void CAdoConnection::ExecuteQuery(LPCTSTR lpszSQL, CDStrs& Fields)
{
	ASSERT(m_pConnection != NULL);
	long  size  = 0,
		  index = 0;
...
/*==================================================================
     name:根据SQL语句查询数据库，获取一个字段值,以字符串的格式返回
===================================================================*/
void CAdoConnection::ExecuteQueryValue(LPCTSTR lpszSQL, CString& value)
{
	ASSERT(m_pConnection != NULL);
	try
	{
		...
		{
			//获取字段值
           _variant_t  thevalue = m_pRs->GetCollect(_variant_t((long)0));
