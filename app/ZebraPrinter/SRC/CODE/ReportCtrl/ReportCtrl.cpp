...
//			- Fixed FindItem for LPARAM as suggested by Armin Z黵cher.
...

{
	...
	//rc.ModifyProperty(RVP_NOITEMTEXT,(LPARAM)"Place your default text here");
...
INT CReportCtrl::InsertColumn(INT nCol, LPCTSTR lpszColumnHeading, int nFormat, int nWidth)

{
	
	RVSUBITEM rvs;
	
	//CString strText = lpszColumnHeading;
	
	
	
	// because we know  LVCFMT_LEFT   == RVCF_LEFT
	
	//					LVCFMT_RIGHT  == RVCF_RIGHT
	
	//					LVCFMT_CENTER == RVCF_CENTER
	
	// so we can write like:
	
	if ((LVCFMT_LEFT == nFormat)||(LVCFMT_RIGHT == nFormat)||(LVCFMT_CENTER == nFormat))
		
		rvs.nFormat = nFormat|RVCF_TEXT;
	...
	rvs.lpszText = (LPTSTR)lpszColumnHeading;
	
	rvs.iWidth = nWidth;
	
	
	
	int nRet = DefineSubItem(nCol, &rvs);
	
	if ( -1 != nRet)
		
		ActivateSubItem(nCol, nCol);
...

{
	...
	// ADDED TO AVOID ASSERT FAIL. kjj 2003.02.20
	
	if (!(iRow >= RVI_EDIT)) return ; 
...
/*BOOL CReportCtrl::SetOnlyDigits(BOOL bOnlyDigits)

  {
  
	m_bOnlyDigits = bOnlyDigits;
	
	  return m_bOnlyDigits;
...

{
		...
		
	{
		...
		// CHEN SHENG ADDED to set the text be selected
...

{
	...
	if(lStyle & WS_BORDER)
		...
		::InflateRect(&lpncsp->rgrc[0], -GetSystemMetrics(SM_CXBORDER), -GetSystemMetrics(SM_CYBORDER));
...

{
	...
	case WM_NCHITTEST:
		...
		return HTTRANSPARENT|CWnd::PreTranslateMessage(pMsg);
	...
	case WM_MBUTTONDOWN:
		
		{
			...
			if(iHitTest == HTCLIENT){
				...
				case WM_LBUTTONDOWN: pMsg->message = WM_NCLBUTTONDOWN;
					
					break;
					
					
					
				case WM_RBUTTONDOWN: pMsg->message = WM_NCRBUTTONDOWN;
					
					break;
					
					
					
				case WM_MBUTTONDOWN: pMsg->message = WM_NCMBUTTONDOWN;
