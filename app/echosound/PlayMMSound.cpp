...
BOOL CPlayMMSound::OpenSoundFile(CString csFileName)
{
	...
	if((DWORD)m_BytesToRead > m_MMCkInfoChild.cksize)
	{
...
BYTE* CPlayMMSound::ReadSoundFile(int*dwBytesRead)
{
	...
	if(dwBytesRead)
		*dwBytesRead = dwRetc;
	char debugBuffer[MAX_PATH] = {0};
	sprintf(debugBuffer,"read %d bytes\n",dwRetc);
	TRACE(debugBuffer);
	return pSoundBuffer;
}

WM_ON_MSG  CPlayMMSound::PlaySoundFile(WPARAM wParam,LPARAM lParam)
{
...
WM_ON_MSG  CPlayMMSound::CloseSoundFile(WPARAM wParam,LPARAM lParam)
{
	if(m_pStopEvent)
...
WM_ON_MSG  CPlayMMSound::OnPlaySoundPtr(WPARAM wParam,LPARAM lParam)
{
	CPlaySound*pPlaySound = (CPlaySound*)lParam;
	m_pPlaySound = pPlaySound;
	return WM_ON_OK;
}
WM_ON_MSG CPlayMMSound::OnEndThread(WPARAM wParam, LPARAM lParam)
{
	CloseSoundFile(0,0);
	::PostQuitMessage(0);
	return WM_ON_OK;
