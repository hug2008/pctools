...
void CGridDlg::OnButtonAdd() 
{
	if(!m_wndCtrl.SaveModifyed(FALSE))
		return;

	int nActiveRow = m_wndCtrl.GetActiveRow();

	// if insert
	if(m_wndCtrl.GetGridRowCount() > 0 && nActiveRow < m_wndCtrl.GetGridRowCount() - 1)
	{
		RECORD_INFO mRec;
		mRec.m_Dedet.SetCurrency(1,0);
		mRec.m_Credit.SetCurrency(1,0);
		m_wndCtrl.m_RowInfo.InsertAt(nActiveRow+1,mRec);

		m_wndCtrl.InsertRows(nActiveRow+1);
		m_wndCtrl.SetActiveRow(nActiveRow+1);
	}
	// if add
	else
	{
		...
		mRec.m_Dedet.SetCurrency(1,0);
		mRec.m_Credit.SetCurrency(1,0);
		m_wndCtrl.m_RowInfo.Add(mRec);

		m_wndCtrl.InsertRows(m_wndCtrl.GetGridRowCount());
	}
}

void CGridDlg::OnButtonDel() 
{
	if(m_wndCtrl.GetGridRowCount() <= 0)
		return;

	int nActiveRow = m_wndCtrl.GetActiveRow();

	m_wndCtrl.m_RowInfo.RemoveAt(nActiveRow,1);
	m_wndCtrl.RemoveRows(nActiveRow);
}
